...
de tungsten plaqué avec de l'or, définissant l'anode. Une différence
de potientiel de $\approx -1500$~V est appliquée entre l'anode et la
...
Le calorimètre électromagnétique est un calorimètre à échantillonage
...
échantilloner l'énergie. Aux extrémités se trouvent deux calorimètres
cuivre-argon~ligquide servant à la calorimétrie hadronique à grand
$|\eta|$ ($1.5 < |\eta| < 3.1$). Dans lerégion $3.1 < |\eta| < 4.9$ se
trouve un autre calorimètre, le \emph{FCal}~\footnote{De l'anglais,
...
pour calorimétrie électromagnétiques) ou en tungsten (deux sections
suivantes, optimisées pour calorimétrie hadronique).

\subsection{Le spectromètre à muon}
\label{sec:lhc_atlas:atlas:mu}

Les particules chargées perdent de l'énergie en intéragissant avec le
...
(un aimant central, deux aux extrémités derrirère les calorimètres) et
...
sur disque des données prises par le détecteur ATLAS nécéssitant
environ 1.3~Mb par événement, un stockage complet corresponderais à
plus de 1000~Tb/s de données enregistrés, ce qui est beaucoup trop
élevé pour être possible. De plus, seulement une petite fraction de
ces événements correspondent aux processus de nouvelle physique
recherchés par la collaboration. Le détecteur ATLAS est donc doté d'un
système de déclancheurs~\footnote{De l'anglais, \emph{triggers}}
permettant de limiter la fréquence d'enregistrement à 200~Hz. Il y a
deux paliers de déclancheurs: un niveau matériel et un niveau
logiciel. Le premier niveau est un ensemble de dispositifs installés
directement sur le détecteur et qui sont activés par plusieurs
signaux, tels que le passage de particules à haute impulsion
transverse dans différentes zones du détecteur ou encore un grand
déficit d'impulsion transverse dans une direction (calculée par
conservation de l'impulsion). Ce premier niveau défini des zones
d'intérêts en $\eta-\phi$ qui sont ensuite relayé à un ensemble de
serveurs sur lesquels des logiciels analysent les zones d'intérêts
pour déterminer si l'événement est intéressant ou non~\cite{collaboration_atlas_2008}.
